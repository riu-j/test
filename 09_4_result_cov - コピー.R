...
project <- "s126"
folder_path <- paste("../notes/", project, "/",
                     "" #sub folder, need "/" at end
                     , project, "_", sep="")
file_name <- paste(folder_path,
                   "test" #file name is here
                   , sep="")
file_extension <- "svg"
...
SetGraphicsDevice(file_extension)
plot(1, 1)
dev.off()
##covariate model
plot_svg = function(){
  svg(paste(folder_path, file_name, "_test.svg", sep=""), width=12, height = 8, bg="transparent")
