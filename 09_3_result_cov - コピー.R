...
{
  proj = "s128"
  folder_name = "G2019S_sex_加算/"
  file_name = "plots_women"
  color = c("blue", "red")
  leg = c("G2019S-", "G2019S+")
  target_cov = "G2019S"
  ...
  dts = c(1 + as.numeric(prms[24]), 1 + as.numeric(prms[23]) + as.numeric(prms[25]) + as.numeric(prms[24]))
  dy = rep(0, numbm)
  cex_size = 0.6
  lwd_size = 0.6
}
...
##covariate model
{
  ...
  for (j in 1:numbm){
    ...
    if(j==1){
      ...
      dt = dts[1]
      plot(graph_main, lwd=4, xlim=xrange, ylim=yrange, col=color[1], add=TRUE)
    }else if(xmax[j]==-1){
      dt = dts[2]
      plot(graph_sub_2, lwd=4, xlim=xrange, ylim=yrange, col=color[2], add=TRUE)
      dt = dts[1]
      plot(graph_sub_2, lwd=4, xlim=xrange, ylim=yrange, col=color[1], add=TRUE)
    }else{
      dt = dts[2]
      plot(graph_sub_score, lwd=4, xlim=xrange, ylim=yrange, col=color[2], add=TRUE)
      dt = dts[1]
      plot(graph_sub_score, lwd=4, xlim=xrange, ylim=yrange, col=color[1], add=TRUE)
    }
  }
