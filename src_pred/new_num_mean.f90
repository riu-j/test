    module sreft
      ...
      real (kind=dpsize), allocatable, save :: offt(:), offt2(:), df_dv(:, :, :), df_time(:, :, :), meanxs(:, :, :), counts(:, :, :)
    contains
      ...
      subroutine initialize(id, meanx, yval, coun, a, b, c, bm, datrec, covt, covy, flag)
...
!other biomarker
      ...
      subroutine readcsv
        ...
        df_dv = undef
        df_time = undef
        open (14, file='s138_data.csv', status='old')
        n = 0
        read(14, '()')
        do
    ...
    subroutine pred(icall, newind, theta, datrec, indxs, f, g, h)
      ...
      real (kind=dpsize), allocatable :: a(:), b(:), c(:), a2(:), b2(:), c2(:), meanx(:), coun(:), covy(:), yval(:, :)
      ...
      flag = 0
      if (newind<=1) call initialize(id, meanx, yval, coun, a, b, c, bm, datrec, covt, covy, flag)
      ...
      do i = 1, numbm
        ...
        call initialize(id, meanx, yval, coun, a2, b, c, bm, datrec, covt, covy, flag)
        call eval(a2, b, c, f2, time, bm, id, covt, covy, flag)
        f2 = f2 + eps(bm)
        g(bm, 1) = (f2 - f) / step

        call initialize(id, meanx, yval, coun, a, b2, c, bm, datrec, covt, covy, flag)
        call eval(a, b2, c, f2, time, bm, id, covt, covy, flag)
        f2 = f2 + eps(bm)
        g(i+numbm, 1) = (f2 - f) / step

        call initialize(id, meanx, yval, coun, a, b, c2, bm, datrec, covt, covy, flag)
