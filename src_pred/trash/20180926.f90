...
REAL(kind = selected_real_kind(8)), parameter :: generalmeanage = 70.0d0, generaltriggerage = 60.0d0, plotmax = 25.0d0, plotmin = 5.0d0
REAL(kind = selected_real_kind(8)), save :: tc(3), epss(3), ww, ma, mi, err, offsetT, m, sum = 0.0d0, wsum = 0.0d0
INTEGER(kind = selected_int_kind(8)), parameter :: NumBM = 3, NumSJ = 400
contains
...
DO i = 1, NumBM
	...
	IF(i == 3)THEN
		IF(c(i) == 0)THEN
			m = (MeanY(i) - a(i)) / b(i)
		ELSE
			m = log((MeanY(i) - a(i)) / (b(i) / c(i)) + 1.0d0) / c(i)
		END IF
	ELSE
		IF(c(i) == 0)THEN
			m = (MeanY(i) - a(i))
		ELSE
			m = log((MeanY(i) - a(i)) * c(i) / b(i) + 1.0d0) / c(i)
		END IF
	END IF
	ma = plotmax - MeanX(i)
	mi = plotmin + MeanX(i)
	tc(i) = (1 - sqrt(1 - 4.0d0 * m)) /  2.0d0
	if(isnan(tc(i)) .eqv. .true.)THEN!下にmでの対応を書いたがそもそもそれをtc(i)でやればよいのでは？
		if(c(i) > 0.0)THEN
			tc(i) = mi
		ELSE
			tc(i) = ma
		...
		ELSEIF(tc(i) > ma)THEN
			...
			ELSE
				tc(i) = tc(i)
	END IF
!	IF(m > 0.25)THEN!上でダメならmに1をかけてこのコードを動かしてみる、あとmとmを比較させて真偽を判定させる。
!		IF(c(i) > 0)THEN
!			tc(i) = mi
!		ELSE
!			tc(i) = ma
...
implicit none
REAL(kind= selected_real_kind(8)) :: THETA(9), DATREC(13), F, G(GPRD, 1), H(HPRD, 1), a(3) ,b(3), c(3), MeanX(3), MeanY(3), Coun(3)
