...
!other biomarker
    ...
    subroutine pred(icall, newind, theta, datrec, indxs, f, g, h)
      ...
      allocate (a(numbm), b(numbm), c(numbm), meanx(numbm), meany(numbm), coun(numbm), cof(numcof))
      id = datrec(5)
      bm = datrec(4)
...
!define covariate
      if (numcof==0) then
        ...
        gend = datrec(5+3*numbm)
        cof = theta(3*numbm+1:3*numbm+1+numcof)
        if (gend==1) then
          covt = (1.0d0+cof(1))
          covy = cof(2)
        else
