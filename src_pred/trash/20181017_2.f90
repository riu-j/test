    module variables
      ...
      real (kind=selected_real_kind(8)) :: epss(1:3) = (/ 0.04d0, 0.0025d0, 0.01d0 /), generalmeanage = 70.0d0, &
        generaltriggerage = 60.0d0, plotmax = 25.0d0, plotmin = 0.0d0
      integer (kind=selected_int_kind(8)) :: numbm = 3, numsj = 400

      real (kind=selected_real_kind(8)), save :: tc(3), offt(400), ww, ma, mi, err, offsett, m, sum, wsum
    contains
...
meanx=datrec(8:10)
coun=datrec(11:13)
      a = theta(1:bm) + eta(1:bm)
      b = theta(bm+1:2*bm) + eta(bm+1:2*bm)
      c = theta(bm+4:3*bm)
